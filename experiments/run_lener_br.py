...
def main():
    ...
    ner_dataset = load_dataset("lener_br")

    label2id = dict()
    id2label =dict()
    label_list = ["O", "B-ORGANIZACAO", "I-ORGANIZACAO", "B-PESSOA", "I-PESSOA", "B-TEMPO", "I-TEMPO", "B-LOCAL", "I-LOCAL", "B-LEGISLACAO", "I-LEGISLACAO", "B-JURISPRUDENCIA", "I-JURISPRUDENCIA"]
    ...
    def relabel(examples):
        
        examples["words"] = [label for label in examples["tokens"]]

        del examples["tokens"]
        
        return examples

    ner_dataset = ner_dataset.map(relabel, batched=True)
