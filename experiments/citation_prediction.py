...
def main():
    ...
    #TODO change name
    config_wandb(model_args=model_args, data_args=data_args, training_args=training_args, project_name=citation-test)
    ...
    ds_dict = {'train': get_dataset(experimental_samples, training_args.do_train, 'train', 'cit'),
               'validation': get_dataset(experimental_samples, training_args.do_eval, 'validation', 'cit'),
               'test': get_dataset(experimental_samples, training_args.do_predict, 'test', 'cit')}

    for k in ds_dict:
        ds_dict[k] = remove_unused_features(ds_dict[k], feature_col, 'cited_rulings')
