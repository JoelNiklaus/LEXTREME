...
def main():
    ...
    # Labels
    label_list = set()
    for labels in pd.DataFrame(train_dataset).labels.tolist():
        for l in labels:
            label_list.add(l)
    ...
    def convert_funct(dataset):
        ...
        dataset_df['text']=dataset_df.text.apply(lambda x: str(x))
        dataset_new = Dataset.from_pandas(dataset_df)
        return dataset_new

    train_dataset = convert_funct(train_dataset)
    eval_dataset = convert_funct(eval_dataset)
    predict_dataset = convert_funct(predict_dataset)
