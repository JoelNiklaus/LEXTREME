...
def main():
    ...
    if training_args.do_train:
        train_dataset = load_dataset("joelito/covid19_emergency_event", split="train", cache_dir=model_args.cache_dir)

    if training_args.do_eval:
        eval_dataset = load_dataset("joelito/covid19_emergency_event", split="validation", cache_dir=model_args.cache_dir)

    if training_args.do_predict:
        predict_dataset = load_dataset("joelito/covid19_emergency_event", split="test", cache_dir=model_args.cache_dir)
    ...
    config = AutoConfig.from_pretrained(
        ...
        finetuning_task=data_args.language+"_covid19_emergency_event",
    ...
    def preprocess_function(examples):
        ...
        batch["labels"] = [[1 if label in labels else 0 for label in list(label2id.keys())] for labels in examples["labels"]]
    ...
    if training_args.do_predict:
        ...
        langs = ["en","da","es","fr","hu","it", "nl", "nn", "no", "pl", "sv", "te"]

        make_predictions_multi_label(trainer=trainer,data_args=data_args,predict_dataset=predict_dataset,id2label=id2label,training_args=training_args,list_of_languages=langs,name_of_input_field='text')
