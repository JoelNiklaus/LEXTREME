...
class ResultAggregator:
    ...
    def get_average_score(self, finetuning_task: str, _name_or_path: str, score: str = None) -> float:
        ...
        if set(results_filtered.seed.unique()).intersection({1, 2, 3}) != {1, 2, 3}:
            logging.error(
                'Attention! It seems you do not have the required seeds 1,2,3 for the finetuning task ' + finetuning_task + ' with the model ' + _name_or_path + '. The average score will be calculated on the basis of incomplete information.')

        if len(results_filtered.seed.tolist()) == 0:
            logging.info(
                'It seems you have duplicate seeds task ' + finetuning_task + ' with the model ' + _name_or_path + " has no meaningful results.")
                ...
                for finetuning_task in self.results.finetuning_task.unique():
                    ...
                    if len(self.meta_infos["task_language_mapping"][finetuning_task]) == 1:
                        ...
                        for aps in self.available_predict_scores:
                            ...
                            if predict_language_mean not in ["",
                                                             np.nan]:  # This is to avoid string values; if there were no scores available I returned an empty string, because 0.0 would be missleading
